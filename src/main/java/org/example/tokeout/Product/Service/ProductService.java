package org.example.tokeout.Product.Service;
...
import org.example.tokeout.Product.DTO.CreateProductDTO;
...
public class ProductService {
    ...
    public MerchantProductVO toMerchantProductVO(Product product, @NonNull Category category) {
        ...
        BeanUtils.copyProperties(product, merchantProductVO);

        // 显式设置或覆盖状态和分类名
        //merchantProductVO.setStatus(ProductStatusEnum.OFF_SALE.getCode());
        merchantProductVO.setCategoryName(category.getCategoryName());
    ...
    public Product toProduct(CreateProductDTO createProductDTO){
        ...
        product.setMerchantId(MerchantContextHolder.getMerchantId());
        product.setIsDeleted(0);//表示未删除
        return product;
    }
    //NOTE:创建商品
    ...
    public MerchantProductVO onShelf(Long productId){
        ...
        if (!product.getStatus().equals(ProductStatusEnum.OFF_SALE.getCode())) {
            throw new BusinessException("当前状态不允许上架");
        }
        // 4. 库存必须 > 0
        ...
        if (product.getPrice() == null || product.getPrice().compareTo(BigDecimal.ZERO) <= 0) {
            throw new BusinessException("价格无效，无法上架");
        }

        // 5. 可选防御：名称非空
        if (product.getProductName() == null || product.getProductName().isBlank()) {
            throw new BusinessException("商品名称为空，无法上架");
        }
        // 6. 更新状态
    ...
    private Product getProduct(Long productId) {
        return productMapper.selectOne(Wrappers.<Product>lambdaQuery().
                ...
                eq(Product::getMerchantId, UserContextHolder.getUserId()));
    }

    //    //NOTE:抽取转换逻辑，让下面专精业务逻辑(经典白写)
//    public PageInfo<MerchantProductVO> toMerchantProductVO(List<Product> products){
//
//        List<MerchantProductVO> list = products.stream().map(product -> {
//            MerchantProductVO merchantProductVO = new MerchantProductVO();
//            BeanUtils.copyProperties(product, merchantProductVO);
//            merchantProductVO.setCategoryName(product.getCategoryName());
//            return merchantProductVO;
//        }).toList();
//
//        //NOTE:第二次提醒，这里不能直接赋值list喵，list已经不是PageInfo对象了喵，要使用下面的方法转换
//        PageInfo<Product> originPageInfo = new PageInfo<>(products);
//        PageInfo<MerchantProductVO> resultPageInfo = new PageInfo<>();
...
//    }
    ...
    public void restoreProduct(@NotNull Long productId) {
        ...
        if (rows == 0) {
            throw new BusinessException("商品不存在或不属于商家");
        }
    }
}
